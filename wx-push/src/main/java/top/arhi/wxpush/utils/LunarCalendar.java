...
public class LunarCalendar {
    ...
    public static JSONObject getLunarBirthday(String date) {
        int month = Integer.parseInt(date.split("-")[0]);
        int day = Integer.parseInt(date.split("-")[1]);
        int nextDay = calcLunarDate(month, day);
        return JsonObjectUtil.packJsonObject(nextDay + "天", "#6EEDE2");
    }
     ...
     * @return 下一次农历生日还要多久
     */
    public static int calcLunarDate(int month, int day) {
        ZoneId zoneId = ZoneId.of("Asia/Shanghai");
        LocalDate localDate = LocalDate.now(zoneId);
        int year = localDate.getYear();
        // 当年年月日
        LocalDate solarDate = lunarToLocalDate(month, day, year);
        // 生日已经过去
        if (localDate.isAfter(solarDate)) {
            // 加一年
            solarDate = lunarToLocalDate(month, day, year + 1);
        }
        // localDate 与 solarDate 相差多少天
        return (int) ChronoUnit.DAYS.between(localDate, solarDate);
    }
    ...
    private static LocalDate lunarToLocalDate(int month, int day, int year) {
        int[] nowOfYear = LunarCalendar.lunarToSolar(year, month, day, true);
        return LocalDate.of(nowOfYear[0], nowOfYear[1], nowOfYear[2]);
    }
