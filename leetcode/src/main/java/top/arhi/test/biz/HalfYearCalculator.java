...
public class HalfYearCalculator extends Calculator {
    @Override
    public int calculateYear(int gap) {
        int year = 1; // 初始年份
        while (gap > 2) {
            gap -= 2; //减去2个半年
            year++;
    ...
    public ContractPeriod strategy(ContractPeriod contractPeriod, BigDecimal bigDecimal, int gap) {
        ...
        if (localDateTime2.isBefore(periodEndDateTimeExpect)) {
            ...
            //判断日期之间有几个自然月
            while (true) {
                LocalDateTime end = localDateTime1.plusMonths(1).minusSeconds(1);
                ...
                if (flag) {
                    ...
                    //最近满一个月时间的结算时间
                    LocalDateTime localDateTimeEnd = localDateTime1.plusMonths(j + 1);
                    //获取实际天数
