...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static final String parseDateToStr(final String DateStr) {
        SimpleDateFormat inputFormat = new SimpleDateFormat(DateUtils.YYYY_MM);
        SimpleDateFormat outputFormat = new SimpleDateFormat(DateUtils._YYYY_MM);
        try {
            //将时间字符串解析为Date对象
    ...
    public static boolean validateDateTime(String dateTime) {
        ...
        if (dateTime == null || "".equals(dateTime)) {
            ...
            try {
                DateUtils.parseDate(dateTime, YYYY_MM_DD_HH_MM_SS);
            } catch (ParseException e) {
                return flag;
            }
    ...
    public static Map<String, Object> isOneMonth(Date begin, Date end) {
        ...
        Integer.parseInt(DateUtils
                .parseDateToStr(DateUtils.YYYY_MM_DD, begin).substring(8, 10));
        Integer beginNum = Integer.parseInt(DateUtils
                .parseDateToStr(DateUtils.YYYY_MM_DD, begin).substring(8, 10));
        Integer endNum = Integer.parseInt(DateUtils
                .parseDateToStr(DateUtils.YYYY_MM_DD, end).substring(8, 10));
    ...
    public static boolean isSameMonth(Date date1, Date date2) {
        boolean flag = false;
        String date1Str = DateUtils.parseDateToStr(DateUtils.YYYY_MM, date1);
        String date2Str = DateUtils.parseDateToStr(DateUtils.YYYY_MM, date2);
...
//        n = 16-n;
    ...
    public static Date endDayPlus(Date end_time, int day) {
        LocalDateTime localDateTime = DateUtils.convertDateToLocalDateTime(end_time).plusDays(day).minusSeconds(1);
        ...
        for (int i = 0; i < months.size(); i++) {
            ...
            if (i == 0) {
                ...
                if (months.size() == 1) {
                    ...
                    //如果没有超过一个月的时间,合同到期的前1s钟作为结束时间
                    endDate = DateUtils.getEnd2(end, true);
                } else {
                    endDate = DateUtils.getEnd(dateStr);
                }
            } else if (i < months.size() - 1) {
