...
public class APITest {
    ...
    @Test
    void next() {
        Integer block = Flux.just(10, 2, 3)
                .next()
                .block();
    ...
    void threadlocal() {
        ...
        Flux.just(1, 2, 3)
                .transformDeferredContextual((flux, context) -> {
                    ...
                    return flux.map(i -> i + "==>" + context.get("suffix"));
                })
                //上游能拿到下游的最近一次数据
                .contextWrite(Context.of("suffix", "哈哈"))
...
//        controller -- service -- dao
    ...
    void paralleFlux() throws IOException {
        // 1000数据，8个线程，每个线程处理100，进行分批处理一直处理结束
        Flux.range(1, 1000)
                .buffer(100)
                .parallel(8)
...
//                .subscribe();
    ...
    void flatrMap() {
        Flux.just("zhang;san;", "li;si;")
                ...
                .log()
                .subscribe(System.out::println);//两个人的名字，按照空格拆分，打印出所有的姓与名
    }
...
//                .log() // 2,4
                .subscribe(System.out::println); //最终消费者;
    }
}
