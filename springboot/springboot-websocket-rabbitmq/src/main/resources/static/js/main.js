...
function onConnected() {
    ...
    stompClient.subscribe('/topic.min', onMessageReceived);

    //额外订阅了mike频道,才能暂时当做自己的频道,别人需要特意往这个频道发送消息,完成 ‘单对单’
    stompClient.subscribe('/user/topic/msg', onMessageReceived);

    // Tell your username to the server
    stompClient.send("/app/chat.addUser", {}, JSON.stringify({sender: username, type: 'JOIN', to: 'all'}))
...
function sendMessage(event) {
    ...
    if (messageContent && stompClient) {
        var chatMessage = {
            sender: username, content: messageInput.value, type: 'CHAT', to: 'all'
